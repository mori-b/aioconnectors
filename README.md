...
In such a case, you need to copy certificates/server to your server (preserving symlinks in case server_ca is false), and certificates/client to your client. That's all you have to do.  
...
When setting server\_ca to true on server side, the client certificates are checked against the server CA, which removes the need to keep all the client certificates on the server. So enabling server\_ca is more scalable on server side.  
...
-**send\_timeout** : maximum time for sending a message between peers. By default 50 seconds. After timeout, the message is lost, the sending peer disconnects, and peers reconnect if enable\_client\_try\_reconnect.  
