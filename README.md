...
### Server example
    ...
    connector_manager = aioconnectors.ConnectorManager(is_server=True, server_sockaddr=server_sockaddr, use_ssl=False, use_token=False,
                                                       ssl_allow_all=True, server_ca=True, connector_files_dirpath=connector_files_dirpath,
...
If you server runs with server\_ca false, then you need the --no-ca argument in create\_certificates, otherwise you don't need it.  
You need to copy certificates/server to your server (preserving symlinks in case server_ca is false), and certificates/client to your client. That's all you have to do.  
...
It is possible to use a CA signed certificate on server, instead of the generated certificate. For that you should :  
-On server side, under "certificates" directory, replace /server/server-cert/server.pem and server.key with your signed certificates. Note that the pem file should contain the whole chain of .crt including the intermediate certificates. You don't need to do that manually, there is a tool that does it :  

    python3 -m aioconnectors replace_server_certificate <custom_server_pem_file_path> [<optional_directory_path>]
    
and in case you want to use again the original server certificate :  

    python3 -m aioconnectors replace_server_certificate --revert
    
-On client side, configure server\_sockaddr with the server hostname, and set client\_cafile\_verify\_server to be the ca cert path (like /etc/ssl/certs/ca-certificates.crt), to enable CA verification of you server certificate.  
...
When server\_ca is true on server side (preferred option), the client certificates are checked against the server CA, which removes the need to keep all the client certificates on the server. So enabling server\_ca is more scalable on server side.  
...
On server side, you can store manually additional default certificates (with their symlink), they must be called defaultN where N is an integer.  
-Other options :  
