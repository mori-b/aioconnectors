...
--ca is needed for a server having server_ca=true (<a href="#classes">4-</a>).  
...
If you server runs with server\_ca true, then you need the --ca argument in create\_certificates, otherwise you don't need it (default).  
...
When using ssl, the recommended approach is to have server\_ca false (default), meaning your server will have a self signed CA certificate that will sign your client certificates. Otherwise with server\_ca false all the generated client certificates are automatically saved on server side.  
...
When server\_ca is true on server side (default), the client certificates are checked against the server CA, which removes the need to keep all the client certificates on the server. So enabling server\_ca is more scalable on server side.  
...
-**server\_ca** : (server only) If set to true (default), the server authenticates client certificates according to its CA only. This is recommended to prevent storing all client certificates on server side. You can always add manually defaultN or CA certificates.  
