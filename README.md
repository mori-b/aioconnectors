...
If you server runs with server\_ca false, then you need the --no-ca argument in create\_certificates, otherwise you don't need it (default).  
...
When using ssl, the recommended approach is to have server\_ca true (default), meaning your server will have a self signed CA certificate that will sign your client certificates. Otherwise with server\_ca false all the generated client certificates are automatically saved on server side.  
